...
This project supports [CSS Modules](https://github.com/css-modules/css-modules) alongside regular stylesheets using the `[name].module.css` file naming convention. CSS Modules allows the scoping of CSS by automatically creating a unique class of the format `[filename]\_[class]\_\_[hash]`.
...
className Button extends Component {
  render() {
    ...
    return <button class={styles.error}>Error Button</button>;
  }
}
```
